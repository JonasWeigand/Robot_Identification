...
static emlrtRSInfo u_emlrtRSI = { 72,  /* lineNo */
  "calc_model",                        /* fcnName */
  "C:\\Users\\Jonas Weigand\\OneDrive\\MATLAB\\21-02-28 Sysident Online Full Robot MEX V7\\functions\\model_payload.m"/* pathName */
};

static emlrtDCInfo emlrtDCI = { 45,    /* lineNo */
  16,                                  /* colNo */
  "calc_model",                        /* fName */
  "C:\\Users\\Jonas Weigand\\OneDrive\\MATLAB\\21-02-28 Sysident Online Full Robot MEX V7\\functions\\model_payload.m",/* pName */
  1                                    /* checkKind */
};
                   ...
                   real_T m_pay, real_T M_inverse[36], real_T C[36], real_T G[6])
{
  ...
  st.site = &h_emlrtRSI;
  calc_model(&st, m_pay_low, q_global, qd, M_inverse, C, G);
  ...
  if (!(m_pay_low == m_pay_high)) {
    st.site = &i_emlrtRSI;
    calc_model(&st, m_pay_low, q_global, qd, M_inverse_low, C_low, G_low);
    d = unnamed_idx_0 - m_pay_low;
    unnamed_idx_0 = m_pay_high - m_pay_low;
    for (i = 0; i < 36; i++) {
      M_inverse[i] = M_inverse_low[i] + (M_inverse[i] - M_inverse_low[i]) * d /
        unnamed_idx_0;
      C[i] = C_low[i] + (C[i] - C_low[i]) * d / unnamed_idx_0;
    }

    for (i = 0; i < 6; i++) {
      G[i] = G_low[i] + (G[i] - G_low[i]) * d / unnamed_idx_0;
    }
  }
...
/* End of code generation (model_payload.c) */
