...
[M_inverse_high, C_high, G_high] = calc_model(m_pay_low, q_global, qd);
...
if m_pay_low == m_pay_high
    ...
    [M_inverse_low, C_low, G_low] = calc_model(m_pay_low, q_global, qd);
...
function [M_inverse_calc, C_calc, G_calc] = calc_model(m_pay_cand, q_global, qd)
        switch m_pay_cand
            case 1
            ...
            otherwise
                % for code generation define this path
                M_calc = eye(6,6);
                C_calc = eye(6,6);
                G_calc = zeros(6,1);
        end
